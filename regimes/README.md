...
What this tells us
The 77.8% baseline is the familiar failure of ordinary recurrent networks. Over 40 time steps, unconstrained matrix multiplication suffers from vanishing and exploding gradients. The Free Chevron physically lost track of the early evidence. It is, as suspected, just a slightly wider scalar soup.
The massive jump to 99.1% for the Complex Chevron perfectly validates the core insight of models like Mamba-3. By constraining the update to a rotation, the network preserves the magnitude of the state vector indefinitely. It remembered the evidence flawlessly across time. But why didn't it score 100%? Because a pure rotation cannot easily shrink a vector to zero. When the Veto token demanded a hard reset, the Complex Chevron had to awkwardly spin its phase angle to approximate an empty state, occasionally leaving "ghost" memories that ruined the final count.
